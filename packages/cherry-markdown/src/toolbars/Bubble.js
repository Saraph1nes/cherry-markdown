...
export default class Bubble extends Toolbar {
  ...
  set visible(visible) {
    ...
    console.log('bubbleStyle', bubbleStyle.display, visible);
    if (visible) {
  ...
  addSelectionChangeListener() {
    this.$cherry.$event.on('onChange', () => {
      ...
      console.log('event = 当编辑区内容变更时自动隐藏bubble工具栏');
      this.hideBubble();
    });
    this.options.editor.addListener('refresh', () => {
      // 当编辑区内容刷新时自动隐藏bubble工具栏
      console.log('event = 当编辑区内容刷新时自动隐藏bubble工具栏');
      this.hideBubble();
    });
    this.$cherry.$event.on('onScroll', () => {
      // 当编辑区滚动时，需要实时同步bubble工具栏的位置
      console.log('event = 当编辑区滚动时，需要实时同步bubble工具栏的位置');
